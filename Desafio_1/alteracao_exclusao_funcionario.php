...
switch ($_POST['form_operacao'])
{
	case "alteracao":
		...
		//exit;
		 ...
		 echo "<script>alert('Alteração realizada com sucesso!');window.location='funcionario.php';</script>";
		 exit;
		 break;

		 case "exclusao":
	  	 $id_funcionario = $_POST['id_funcionario'];
   		 $ComandoSQL = "delete from funcionario where id_funcionario = '$id_funcionario'";
   		 $result = mysql_query($ComandoSQL, $vConexao);
		 echo "<script>alert('Exclusão realizada com sucesso!');window.location='funcionario.php';</script>";
		 exit; 
  	     break;
}

$ComandoSQL = "select * from funcionario where id_funcionario = '" . $id_funcionario . "'";
$result = mysql_query($ComandoSQL, $vConexao);
if (mysql_num_rows($result)==0)
{
	echo "<script>alert('Funcionario não encontrado!');window.location='funcionario.php';</script>";
	exit;
}
...
<script LANGUAGE="JavaScript">
  function define_operacao(operacao){
    if (operacao == "alt") {
       document.form_alteracao_exclusao_funcionario.form_operacao.value = "alteracao";
    }
    if (operacao == "exc") {
       document.form_alteracao_exclusao_funcionario.form_operacao.value = "exclusao";
    }
    document.form_alteracao_exclusao_funcionario.submit();
  }
  </script>
</head>
<body>
...
<div id="conteudo">
	...
	<ul id="nav">
		...
		<li><a href="consulta.php">Consulta (Alteracao e Exclusao)</a></li>
	</ul>
