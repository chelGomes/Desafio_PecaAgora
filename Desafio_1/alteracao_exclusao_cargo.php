...
switch ($_POST['form_operacao'])
{
	case "alteracao":
		...
		//exit;
		 ...
		 echo "<script>alert('Alteração realizada com sucesso!');window.location='cargo.php';</script>";
		 exit;
		 break;

		 case "exclusao":
	  	 $cargo_id = $_POST['cargo_id'];
   		 $ComandoSQL = "delete from cargo where cargo_id = '$cargo_id'";
   		 $result = mysql_query($ComandoSQL, $vConexao);
		 echo "<script>alert('Exclusão realizada com sucesso!');window.location='cargo.php';</script>";
		 exit; 
  	     break;
}

$ComandoSQL = "select * from cargo where cargo_id = '" . $cargo_id . "'";
$result = mysql_query($ComandoSQL, $vConexao);
if (mysql_num_rows($result)==0)
{
	echo "<script>alert('Cargo não encontrado!');window.location='cargo.php';</script>";
	exit;
}
...
<script LANGUAGE="JavaScript">
  function define_operacao(operacao){
    if (operacao == "alt") {
       document.form_alteracao_exclusao_cargo.form_operacao.value = "alteracao";
    }
    if (operacao == "exc") {
       document.form_alteracao_exclusao_cargo.form_operacao.value = "exclusao";
    }
    document.form_alteracao_exclusao_cargo.submit();
  }
  </script>
</head>
<body>
